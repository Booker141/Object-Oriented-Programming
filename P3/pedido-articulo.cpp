...
ostream& operator <<(ostream& os, const LineaPedido& LineaP) {
	os << fixed << setprecision(2) << LineaP.precio_venta() << " euros\t" <<
	   LineaP.cantidad();
	return os;
}
...
ostream& Pedido_Articulo::mostrarDetallePedidos(ostream& os) const {
	...
	os << fixed;
	os << "TOTAL VENTAS " << setprecision(2) << precio << " euros" << endl;
	return os;
}
...
ostream& operator<<(ostream& os, const Pedido_Articulo::ItemsPedido& ItemsP) {
	...
	for(i = ItemsP.begin(); i != ItemsP.end(); ++i) {
		...
		os << " " << i->second.precio_venta() << "euros\t";
	...
	os << "===================================================" << endl;
	os << fixed;
	os << setprecision(2) << precio << " euros" << endl;
	return os;
}
...
ostream& operator<<(ostream& os, const Pedido_Articulo::Pedidos& pedido) {
	...
	os << "====================================================" << endl;
	os << fixed;
	os << setprecision(2) << precio << " euros\t" << total <<endl ;
	return os;
